...
function AjouterDemande($cnx, $data)
{
    // Prepare each field from the $data array
    $gender     = $data['gender'];
    $prenom     = $data['prenom'];
    $nom        = $data['nom'];
    $email      = $data['email'];
    ...
    $enfant = isset($data['enfants']) && $data['enfants'] === 'oui' ? 1 : 0;
    echo "<script>console.log('Demande ajoutée avec succès: " . $data . "');</script>";

    $req = "INSERT INTO demande(user ,gender, prenom, nom, email, postal, ville, telphone, motivation, family, enfant)
            VALUES 
            (1 ,'" . $gender . "', '" . $prenom . "', '" . $email . "', '" . $postal . "', '" . $ville . "', '" . $tel . "', '" . $motivation . "', '" . $family . "', '" . $enfant . "')";
